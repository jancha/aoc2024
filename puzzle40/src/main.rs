    ...
    loop {
        ...
        for dx in -20..=20_isize {
            for dy in -20..=20_isize {
                ...
                if (map_binary[prev_index] > 0 || prev_index == *start)
                    && map_binary[prev_index] < cost + path_len
                {
                    let delta = cost - map_binary[prev_index] - path_len;

                    if delta < min_cheat_gain {
                        continue;
                    }
...
#[test]
fn test_1() {
    assert_eq!(analyze("test.txt", 50), 285);
    assert_eq!(analyze("test.txt", 70), 41);
    assert_eq!(analyze("test.txt", 76), 3);
}
